...
export default function MotionCompare(props) {
    ...
    function dots(color=0x33eeb0, size=0.02) {
        return new THREE.Mesh(new THREE.BoxGeometry(size, size, size), new THREE.MeshBasicMaterial({color: color}));
    }
    ...
    function plotPose() {
        ...
        for (let j of joints) {
            ...
            for (let landmark of poseArr) {
                ...
                const pos = posePointsToVector(landmark[POSE_LANDMARKS[j[1]]], landmark[POSE_LANDMARKS[j[0]]]);
