...
BVH.Reader.prototype = {
				...
				if (this.readyState == 4) {
					...
					img.onload = function (e) {
						...
						for (i = 0, l = d.length; i < l; i += 4) {
							pix = d[i];
							if (pix < 96)
								string += String.fromCharCode(pix + 32);
						}
				...
				if (node.children.length) {
					...
					if (bone.name === "Head")
						bone.scale.set(
							this.boneSize * 2,
							this.boneSize * 2,
							BVH.DistanceTest(bone.position, target) *
								(this.boneSize * 1.5)
						);
					else
						bone.scale.set(
	...
	animate: function () {
		...
		for (var i = 0, len = ref.length; i < len; i++) {
			...
			switch (ch.prop) {
				case "Xrotation":
					this.autoDetectRotation(
						ch.node,
						"X",
						parseFloat(this.data[n])
					);
				...
				case "Xposition":
					if (isRoot)
						...
						ch.node.position.x =
							ch.node.offset.x + parseFloat(this.data[n]);
					break;
				case "Yposition":
					if (isRoot)
						ch.node.position.y =
						...
						ch.node.position.y =
							ch.node.offset.y + parseFloat(this.data[n]);
					break;
				case "Zposition":
					if (isRoot)
						ch.node.position.z =
						...
						ch.node.position.z =
							ch.node.offset.z + parseFloat(this.data[n]);
					break;
			}
	...
	update: function () {
		if (this.play) {
			this.frame =
				(((Date.now() - this.startTime) / this.secsPerFrame / 1000) *
					this.speed) |
				0;
