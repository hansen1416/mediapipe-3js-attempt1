...
export default class Silhouette3D {
	...
	applyPose(pose3D) {
		...
		for (let name of this.limbs) {
			...
			const parent_name = this[name].parent_name;

			if (qs[parent_name]) {
				const q = qs[name]
					.clone()
					.multiply(qs[parent_name].clone().invert());

				if (name === "thigh_l") {
					console.log(parent_name, qs[name], qs[parent_name], q);
				}
