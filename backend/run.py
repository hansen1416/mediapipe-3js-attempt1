...
class PreprocessVideo():
    ...
    def frame_find_result_file(self, frame_index):
        ...
        suffix = str(frame_index - residual) + '-' + str(frame_index + (3000 - residual))
    ...
    def plot_pose_for_result(self, frame_index):
        ...
        self.plot_world_pose(pose_lm.landmark, os.path.join('tmp', 'pose-res-world-{}.png'.format(frame_index)))
        self.plot_viewport_pose(pose_wlm.landmark, os.path.join('tmp', 'pose-res-viewport-{}.png'.format(frame_index)))
    ...
    def find_angle_between_vectors(self, vec1, vec2):
        ...
        mag = ((vec1[0] ** 2 + vec1[1] ** 2 + vec1[2] ** 2) ** 0.5) * ((vec2[0] ** 2 + vec2[1] ** 2 + vec2[2] ** 2) ** 0.5)
    ...
    def compare_poses(self, frame1, frame2):
        ...
        logger.info("============== Viewport landmark, frame {} vs {}".format(frame1, frame2))
        self.plot_degrees(lm1.landmark, lm2.landmark)
        logger.info("============== World landmark, frame {} vs {}".format(frame1, frame2))
    ...
    def plot_degrees(self, landmark1, landmark2):
        ...
        for f in Extremities._fields:
            ...
            logger.info("{} shifted degrees {}, radius {}".format(f, math.degrees(radius), radius))
